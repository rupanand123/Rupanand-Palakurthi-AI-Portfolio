...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (!key || key === "MY_GEMINI_API_KEY" || key.trim() === "") {
      ...
      return res.json({ text: responseText, mode: "offline" });
    }
    ...
    res.json({ text: reply, mode: "online" });
  } catch (err: any) {
    console.error("Gemini API Error:", err);
    res.status(500).json({ error: err?.message || "Internal core server error" });
  }
});
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`JARVIS Grid Server actively listening on http://localhost:${PORT}`);
  });
}
